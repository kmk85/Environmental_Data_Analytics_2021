...
# default for ggplot is gray background with white lines
...
# will start changing the theme (background is now white with gray lines and black axes)
...
# x and y axes lines but no background lines
...
* text size (default for ggplot is size 11)
* axis label colors (default is black)
* legend position and justification (defualt position is right)
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# adding more particularities to my theme; increase font size and change text elements (axes text to black)
...
# Adding horizontal lines at 50 and 100. lty = 2 makes dotted line; alpha adjusts transparency of points and size changes the size; adding text to plot and specifying the position, hjust is the horizontal justification of the text; changing the scale on the x-axis (need to specify a range of values to plot); changing the representation of x axis label; changing the label of the y axis; changing the angle of axis text
...
# Did this plot in part 1; different shape for each lake. Color changes with depth
NvsP <-
...
# adding transparency of the points and the size
# changing the shapes of the points (15 and 17); can search for the scale_shape_manual to find the numbers that refer to different shapes
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
...
library(cowplot)
# rel_heights adjusts the space for each plot (can size the plots)
