 ...
 * Spatial data structures: geometry (a set of spatial coordinates), sfg (spatial feature geometry = one row within the geometry column; i.e. a set of coordinates with coordinate reference system), sfc (spatial features column = geometry column), sf (spatial feature/dataframe) objects
...
#Check the CRS - the huc8 dataset uses a different CRS than the counties dataset
st_crs(huc8_sf)
st_crs(counties_sf)
...
#Summarize on DWQ Basin value ###Need to finish code
huc2_sf <- huc8_sf %>% 
  group_by(DWQ_Basin) %>% 
  summarize(
    ACRES = sum(ACRES),
