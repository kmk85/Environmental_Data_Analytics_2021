...
# We can extract values in vectors by their position.
vector1[3] # 

# What happens if we mix types (numeric and character)? Defaults to the braodest category
vectorx <- c(1, "one")

# Matrices ----
# Matrices have all of the same data type. The default is to list down
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # 
matrix1

# You can tell R to list across
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #
matrix2

# Instead of using row and column numbers, assigning names
...
# Various ways you can extract values out of matrices
matrix1[4, ] #
matrix1[ , 3] #
matrix1[c(12, 14)] #
matrix1[c(12:14)] #

# slice of a row and column
matrix1[2:4, 1:3] #

# How to develop a matrix with values, rows, and column names
cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
...
# Lists ---- 
# We can mix the data types in lists
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1

# Why use two brackets? It will return the data type versus a list. See "class" command below
list1[[2]]
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # Updating the names of the dataframe

# like a matrix, can retrieve information from a dataframe
# This will give us rows 1 and 2, and all columns
dataframe1[1:2,] # 
dataframe1[c("ID","Passed")] # 
dataframe1$ID # Returning the vector "ID"

# Can hightlight something in your code and click F1 for the "help" page

```
Question: How do the different types of data appear in the Environment tab?

> Answer: Things that are more than one dimension shows in the data tab, where things that are one dimention show up in the values tab

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: You see the column names
...
high.F <- c(50, 53, 50, 56, 54, 51, 52, 40, 37, 35)
low.F <- c(40, 31, 29, 27, 40, 30, 33, 25, 20, 22)
...
# convert F vectors to C
high.C <- celsius(high.F)
low.C <- celsius(low.F)
...
temp.dat <- data.frame(high.F, low.F, high.C, low.C)

# If I wanted to change the names of the columns
# names(temp.dat) <- c('hi_F', 'lo_F', 'hi_C', 'lo_C')
...
# this will convert a string to a date object (will have UTC next to it in Environment)
