...
#Read our EPA points into a spatial dataframe (using the st_as_sf command)
#crs= 4269 is WGS 1984
...
# use st_read to read in a shapefile;
...
#Reveal the extent of this dataset via the st_bbox() function; creates a spatial bounding box around all of the features
st_bbox(counties_sf)
...
#Create a map ### NEED HELP WITH THIS
...
# colclasses = giving a list of column names and importing as a factor
...
huc8_sf_wind <- huc8_sf %>% 
  left_join(wind_energy_huc8, by = c('HUC_8'))
...
#Select intersecting counties using matrix subsetting (1st way to do this)
neuse_intersect_1 <- counties_sf_utm[neuse_sf,]

#Select intersecting counties using the `filter()` command (2nd way to do this)
...
```{r EXERCISE: Clipping}
#Select the Triangle County from the sf_counties dataframe
...
# Leaflet likes to deal with datasets that are in WGS 84 because that is what its base layers are in
...
# can add multiple items and change basemap using website above
