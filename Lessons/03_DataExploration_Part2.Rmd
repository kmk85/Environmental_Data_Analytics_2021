...
colnames(USGS.flow.data) # looking at the column names
str(USGS.flow.data) # looking at the structure of the data
dim(USGS.flow.data) # looking at the dimensions of the data

# Check our date column; .csv files do not bring in dates as a date field - they bring it in as a character field
...
# taking out all of the NAs
ggplot(na.omit(USGS.flow.data), aes(x = discharge.mean.approval)) +
  geom_bar()

# gg in ggplot = grammer of graphics
# aes is short for aesthetics; x = categorical sub-dataset; geom = the type of geometry we want to plot (i.e. bar graph in this case)
# Bar plots are useful for categorical data; looking at the counts in each category
...
# This histogram is showing the distribution of the values of mean discharge
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

# The "binwidth" argument cahnges the bin width
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# and the "bins" argument changes the # of bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# How do you determine the default # of bins or default bin widths? Look at  the help page for geom_histogram();
# Should generally use betwee 5 and 20 bins

# The "scale_x_continuous command changes the x scale of the plot; "zooms in" to a section of the data
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# This data is less skewed/more normally distributed; Play around with the number of bins to look at patterns in the data
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# Location of ggplot cheat sheet in R: Help -> Cheatsheets -> Data Visualization with ggplot2
...
# Can show a histogram and frequency line graph at the same time
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = gage.height.mean), bins = 50) +
  geom_freqpoly(aes(x = gage.height.mean), bins = 50)

# setting the color to a categorical value; modifying the aesthetics to color the unique values in gage.height.mean.approval (A and P)
ggplot(USGS.flow.data) +
...
# x is the numeric value in which we want to look at the distribution; Showing the distribution of the mean gage height values
# A has a tight distribution with a lot of outliers
# Looking at categorical and numeric data
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

# Taking different slices of our data and looking at the distribution; Slicing the data with "cut_wiidth" Showing the variance/distributions across different slices of data.
# There is a tight distribution of data near low values of mean gage height, but a much braoder distribution at larger values of of mean gage height
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

# Violin plot looks at the range and the distribution of values; Shows IQRs, but also shows how many data points are distributed where. 
ggplot(USGS.flow.data) +
...
# Can use scatterplots to look at patterns in the data; each point is a row in the data. 
ggplot(USGS.flow.data) +
  geom_point(aes(x = discharge.mean, y = gage.height.mean))

# datetime is a hybrid between categorical and continuous; Showing mean discharge at different times. Can see that there is a big break where we don't have data for the site. 
ggplot(USGS.flow.data) +
