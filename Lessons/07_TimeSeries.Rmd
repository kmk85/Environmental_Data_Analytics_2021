...
library(Kendall)
#install.packages("tseries") # not using this today, but can use as additional resource
library(tseries)
...
# The first thing you do in TSA is plot your variable over time and look at it; Can clearly see that there is a huge gap in the data (1971 - 1985)
...
```{r}
# Because there is a hug gap in the data, we will split the data into early and late years
...
3. An **error** or **random** component, which makes up the remainder of the time series after other components have been accounted for. This component reflects the noise in the dataset. (the component that cannot be explained by trend or seasonality)
...
```{r}
# Analyzing the discharge.mean over time; need to define the starting time by year and month; also need to specify the frequency - 365 means it will consider every day. 
EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, start = c(1985,1), frequency = 365)

# If I want to look at the time series object; Looking at the first 10 elements
...
```{r}
# Need to transform data into a TSA (i.e., use the ts() function) before using the stl() function
?stl
# Performs a seasonal decomposition of the time series; ***first argument in the function needs to be a univariate time series, second function is the seasonal window, but if you specify this when you create the time series, you just have to put s.window = "periodic"
...
# will repeat itself every 365 observations
# Some periods, the trends increase and some periods the trend decreases, so it is not a monotonic dataset

# If you want to find a specific element of your decomposed object, can look at EnoDischarge.late_Decomposed$time.series
# when you sum the seasonal, trend, and remainder columns, you get the values of the observed data; will create a dataframe with these three columns
# Also provides the start date, end date, and frequency
...
# Visualize how the trend maps onto the data (plotting the components over time)
# adding lines for the observed and trend components and a line for the yintercept at 0
# red line is the trend over time
...
# seasonal component goes below y=0
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
...
```{r}
# Looking at the first 6 observations of the wind_data dataset
head(wind_data)
# summarizing the last column of the dataset (called AWND) and you can see there there is 1 NA
summary(wind_data$AWND)

# Adding new column with no missing obs, just for illustration purpose
# In real applications you will simply replace NAs (i.e., do the replacement on the original column)
...
# Can see where there was a missing observation (red line)
```
...
# Using the first() function to make objects for the month year and first year
...
# seasonal component is much easier to spot becasue it repeats every 12 observations; overall decreasing trend (by eye, but need a statistical test to prove this)
...
# p-value < 0.05, so the reject the null -> there is a trend in the data
...
# shows results for each season of the year. Can see the values of "S"
...
> Answer:There is a negative slope, so there is a decreasing trend of the average wind speed over time.
