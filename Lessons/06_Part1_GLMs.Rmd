...
# providing the data frame and then don't need to use the $ sign
irradiance.regression <- lm(data = PeterPaul.chem.nutrients, irradianceWater ~ depth)
summary(irradiance.regression)
# residuals will range from -458.9 to 23813.0 
# coefficients: intercept = 486.818, depth coefficient = slope of the regression line that is trying to find the relationship between depth and irradianceWater is -95.890 (negative relationship)
# Standard errors for coefficients
# p-value (our confidence level) is less than 0.05 (can explain irradiance by depth)
# Want RSE to be as small as possible
# degrees of freedom will be associated with the # of observations in our sample and the # of variables we are considering in our linear regression
# R-squared tells us how much variability is being explained: depth explains around 30% of the variability in irrandiance


# Correlation
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth)
# it is a negative correlation. The strength of the correlation is .55, which is a strong correlation between the two variables
```
...
# dividing plot screen into 4 cells (2x2); mar() defines the size of the plots
par(mfrow = c(2,2), mar=c(4,4,4,4))
plot(irradiance.regression)
# Plot 1: looking for symmetry based on the red line; want the red line to be as flat as possible; one outlier obervation - may want to remove that observation
# Plot 2: Normal Q-Q plot; dashed line corresponds to the Q-Q of the normal distribution and the circles correspond to the Q-Q for our residuals. If we want our residuals to follow a normal distribution, we want our circles to follow the dashed line
# Plot 3: comparing fitted values with the square of standardized residuals. Looking for symmetry with respect to the red line. 
# Plot 4: This plot will help us look at potential outliers. Look for drastic departures from the solid red line = poor model fit. Outliers will typically be outside of cook's distance (the dashed red lines)
...
# using ylim does not remove the outlier values, but it changes the scale of the y-axis; There is not a linear relationship betwen x and y, so can try to log-transform 
...
```{r, fig.height = 3, fig.width = 4}
# filtering the dataset; In order to take the log, need to remove the 0's. Also want to remove the outlier, so getting rid of any irradianceWater > 5000.
...
# relationship is still negative; p-values are still smaller than confidence levels, so the intercept and slope are statistically different from 0; RSE has been reduced; R-squared value has increased, so now depth explains more variability in irradiance 


par(mfrow = c(2,2), mar=c(4,4,4,4))
plot(irradiance.regression2)
par(mfrow = c(1,1))
# removed outlier, so easier to see symmetry along red line

# Add a line and standard error for the linear regression
# GOing to take the log of irradiance (scale_y_log10), so the y-axis will be log-transformed
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
...
# se = FALSE eliminates the confidence intervals on the graph
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point() +
...
# residual plots of the regression
par(mfrow = c(2,2), mar=c(4,4,4,4))
plot(TPregression)
par(mfrow = c(1,1))
...
library(corrplot)
# creating a new dataframe with the columns of interest and omit all NAs
PeterPaulnutrients <- 
...
# the thinner the line, the higher the correlation; blue shows positive correlation

# a way of showing the value of the correlation with the ellipses
...
# consider all the nutrients as explanatory variables
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4)

#Choose a model by AIC in a Stepwise Algorithm
step(TPAIC)
# Starting point = full regression with all variables; want the smallest AIC; step() fuction alalyzes the function after removing explanatory variables one-by-one; best regression is considering oxygen, temp, and nitrogen; also give coefficients for each of the explanatory variables
