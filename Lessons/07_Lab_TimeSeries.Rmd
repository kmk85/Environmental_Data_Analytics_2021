...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c(NA,"Month",NA)," ") %>% 
...
# using sum() gives you the number of NAs
sum(is.na(BRA_complete))

#can also look at summary() to see if there are any NAs; there aren't any so it doesn't show any NAs
...
rain_plot <- ggplot(BRA_complete, aes(x = Date, y = Rainfall)) +
  geom_smooth(method = "lm") +
  geom_line()
print(rain_plot)
#a plot with no trend is stationary

temp_plot <- ggplot(BRA_complete, aes(x = Date, y = Temperature_C)) +
  geom_smooth(method = "loess") +
  geom_line() +
  labs(y = "Temperature")
print(temp_plot)

# other geom_smooth() methods = instead of "lm" can change to "loess"
...
rain_ts <- ts(BRA_complete$Rainfall, start = c(first.yr, first.mo),
              frequency = 12)
temp_ts <- ts(BRA_complete$Temperature_C, start = c(first.yr, first.mo),
              frequency = 12)
