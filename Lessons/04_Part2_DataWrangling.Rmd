...
# Reading in the Nutrients .csv and changing all of the characters to factors so we can see the summary of character/factor columns
...
# the summary is still showing the values for all of the lake names
...
# use the as.date function to create a date object for our factor
NTL.nutrient.data.PeterPaul$sampledate <- as.Date(NTL.nutrient.data.PeterPaul$sampledate, format = "%m/%d/%y")

# Can use the pipe method to perform all of the operations you want on the Nutrient PeterPaul dataset
...
# Remove columns that are not of interest for analysis; selecting the columns you want to keep
NTL.phys.data.PeterPaul.subset <- select(NTL.phys.data.PeterPaul, 
                                         lakename:irradianceDeck)

# Selecting the names of the columns you want to keep
...
# removing the rows with NA in Concentration; not creating a new dataframe
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))

# asking R to count, for each nutrient, how many observations there are; specified a column
...
# pivot_wider: taking a long datasets and transforming some of the rows into columns, so making the dataset wider than it was
# transforming this dataset back to what it was (i.e. opposite transformation from pivot_longer)
...
# example of 'separate': spliting a date (year, month, and date) into 3 separate columns
# can use 'unite' in place of 'separate' to combine columns
...
# the datasets had 6 common columns, 4 unique columns from the physical dataset and 5 unique columns from the nutrients data (resulting dataset will have 15 variables/columns)
...
```{r}
# creating a list with the list.files function; need to specify a path where all the files you want to read are located; also specify the pattern (i.e., all the files that end with .csv; using the full names of the files)
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE)
# LitterFiles has the info on all 20 datasets. It copies to an object
LitterFiles

# Want to use a pipe (order of functions/commands to perform);
Litter <- LitterFiles %>%
  ldply(read.csv)
```
...
# Have the same # of observations that we had in Litter (the longer dataset) but more columns because mixing the 23 variables in trap minus the 3 columns they have in common
...
```{r}
# First thing to do in the pipeline is filter by depth, then group by lakename and month, then filter NAs for temp, tn_ug, and tp_ug, then create summaries 
